...
\begin{itemize}
    ...
    \item \textbf{Desvantagem:} Aumenta o custo computacional, ao gerar mais uma consulta e mais uma
        tabela para ser feito dentro do sistema.

    \item \textbf{Solução alternativa:} O mapeamento de atributos multivalorados também torna possível
        mapeá-lo na tabela da entidade a qual ele pertence, porém é possível que um cidadão
        tenha diversas alergias, gerando assim, um armazenamento maior e possíveis
        incoerências.
\end{itemize}
...
seria necessário fazer JOINS.
    ...
    \item \textbf{Solução alternativa:} Uma opção seria tratar os atributos de unidade de saúde e
Enfermeiro como valores de atributo dentro da entidade unidade saúde, sem a
necessidade de trabalhar com mais duas tabelas para isso.
...
\begin{itemize}
    \item \textbf{SGBD:} PostgreSQL. Escolhido devido à sua robustez, suporte a integridade referencial e conformidade com padrões SQL.
    \item \textbf{Linguagem de Backend:} Python com framework Flask/FastAPI (ajustar conforme uso). Utilizado para criar a API que comunica o banco de dados com a interface.
    \item \textbf{Interface:} HTML/JS simples para demonstração dos requisitos.
    \item \textbf{Conexão:} Biblioteca \texttt{psycopg2} para conexão segura e execução de queries parametrizadas.
    \item \textbf{Ambiente:} Docker, garantindo a portabilidade da aplicação e do banco de dados.
\end{itemize}
...
% --- Divisão Relacional ---
\subsubsection{Consulta com Divisão Relacional}
\textbf{Contexto:} Esta consulta identifica cidadãos que tomaram \textit{todos} os tipos de vacinas disponíveis no sistema. Utilizamos a lógica de \texttt{NOT EXISTS} aninhado para simular a divisão relacional (álgebra relacional).

\textbf{Arquivo:} \texttt{src/back/queries/consultas\_queries.py}

\begin{lstlisting}[language=SQL, caption=Consulta de Divisão Relacional (Cidadãos com esquema vacinal completo)]
SELECT c.cpf, c.nome
FROM cidadao c
WHERE NOT EXISTS (
    -- Seleciona todas as vacinas existentes
    SELECT v.lote, v.cod
    FROM vacina v
    EXCEPT
    -- Remove as vacinas que o cidadão JÁ tomou
...
\subsubsection{Busca de Cidadão (Parametrizada)}
\textbf{Contexto:} Busca os dados básicos de um cidadão pelo CPF. O uso de parâmetros (\texttt{\%s}) é essencial para prevenir ataques de SQL Injection na aplicação Python.

\textbf{Arquivo:} \texttt{src/back/queries/cidadao\_queries.py}

\begin{lstlisting}[language=Python, caption=Query parametrizada para busca segura]
...
\subsubsection{Consulta de Histórico Completo}
\textbf{Contexto:} Utiliza múltiplos \texttt{LEFT JOIN} para agregar informações de consultas, exames e receitas em uma única visão cronológica para o prontuário.

\textbf{Arquivo:} \texttt{src/back/queries/consultas\_queries.py}

\begin{lstlisting}[language=SQL, caption=Histórico Médico Unificado]
SELECT
    c.nome as cidadao,
...
LEFT JOIN exame e ON cons.cidadao = e.cidadao 
    AND cons.medico = e.medico 
    AND cons.data = e.data
LEFT JOIN receita r ON cons.cidadao = r.cidadao 
    AND cons.medico = r.medico 
    AND cons.data = r.data
WHERE c.cpf = %s
ORDER BY cons.data DESC;
\end{lstlisting}

\section{Conclusão}
O desenvolvimento do Prontuário Eletrônico do Cidadão permitiu aplicar na prática os conceitos de modelagem relacional e integração de aplicações. A maior dificuldade encontrada foi a modelagem correta da generalização/especialização dos profissionais de saúde e a implementação da divisão relacional em SQL puro.
